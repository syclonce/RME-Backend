...
class DischargeMedicationReconciliationControllerTest extends TestCase
{
    ...
    {
        $this->actingUser();
        $record = DischargeMedicationReconciliation::factory()->create(['status' => 'draft']);

        $this->putJson("/api/v1/discharge-med-reconciliations/{$record->id}", ['status' => 'completed'])
    ...
    {
        $this->actingUser();
        $record = DischargeMedicationReconciliation::factory()->create(['status' => 'completed']);

        $this->putJson("/api/v1/discharge-med-reconciliations/{$record->id}", ['status' => 'draft'])
            ->assertStatus(422);
    }
    ...
    public function test_it_rejects_invalid_status(): void
    {
        ...
        $this->putJson("/api/v1/discharge-med-reconciliations/{$record->id}", ['status' => 'teleported'])
            ->assertStatus(422);
    }
    ...
    public function test_it_rejects_create_when_medical_record_is_finalized(): void
    {
        ...
        $this->postJson('/api/v1/discharge-med-reconciliations', [
            'visit_id' => $visit->id,
            'reconciled_by' => $reconciledBy->id,
        ])->assertStatus(422);

        $this->assertDatabaseCount('discharge_medication_reconciliations', 0);
    }
}
