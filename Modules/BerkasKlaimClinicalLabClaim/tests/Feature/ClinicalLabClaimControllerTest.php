...
class ClinicalLabClaimControllerTest extends TestCase
{
    ...
    public function test_it_submits_a_draft_claim(): void
    {
        ...
        $this->patchJson("/api/v1/clinical-lab-claims/{$claim->id}/transition", ['status' => 'submitted'])
            ->assertOk()
    ...
    {
        $this->actingUser();
        $claim = ClinicalLabClaim::factory()->create(['status' => 'submitted']);

        $this->patchJson("/api/v1/clinical-lab-claims/{$claim->id}/transition", ['status' => 'approved'])
            ->assertOk()
