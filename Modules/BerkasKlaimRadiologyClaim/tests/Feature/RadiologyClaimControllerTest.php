...
class RadiologyClaimControllerTest extends TestCase
{
    ...
    public function test_it_submits_a_draft_claim(): void
    {
        ...
        $this->patchJson("/api/v1/radiology-claims/{$claim->id}/transition", ['status' => 'submitted'])
            ->assertOk()
    ...
    {
        $this->actingUser();
        $claim = RadiologyClaim::factory()->create(['status' => 'submitted']);

        $this->patchJson("/api/v1/radiology-claims/{$claim->id}/transition", ['status' => 'approved'])
            ->assertOk()
